...
public class EnemyManager : IInitializable, IDisposable, ITickable
{
    ...
    private void Spawn(GameObject prefab, Vector3 position)
    {
        ...
        if (obj.TryGetComponent(out IEnemy enemyInterface)) currentEnemy = enemyInterface;
    }
    ...
    {
        if (currentEnemy is MonoBehaviour enemyComponent) UnityEngine.Object.Destroy(enemyComponent.gameObject);
        currentEnemy = null;
